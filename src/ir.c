...
void wiiuse_set_ir_sensitivity(struct wiimote_t *wm, int level)
{
    ...
    WIIMOTE_DISABLE_STATE(wm, (WIIMOTE_STATE_IR_SENS_LVL1 | WIIMOTE_STATE_IR_SENS_LVL2
...
void interpret_ir_data(struct wiimote_t *wm)
{
...
#ifdef WITH_WIIUSE_DEBUG
    {
        ...
        for (i = 0; i < 4; ++i)
            if (dot[i].visible)
            {
                WIIUSE_DEBUG("IR[%i][order %i] (%.3i, %.3i) -> (%.3i, %.3i)", i, dot[i].order, dot[i].rx,
                             dot[i].ry, dot[i].x, dot[i].y);
            }
